...
const UserMessages = () => {
  const sms = JSON.parse(localStorage.getItem("ewebal_messages")!);
  const { data, isLoading } = usePopulateSmsHistory(sms);

  return (
    <>
      <Div>
        <Top>
          <h1>User's Successful message</h1>
                  ...
                  data?.map((el: any, idx: number) => (
                    <Tbody key={idx}>
                      <tr>
                        ...
                        >
                          <Link
                            to={`/admin-dashboard/manage-all-users/user-messages/${sms}/more-about-sms/${el?._id}`}
                            onClick={() =>
                              localStorage.setItem(
                                "ewebal_messages_more",
                                JSON.stringify(el)
                              )
                            }
                          >
